use ethers_core::types::{Filter, BlockId};
...
pub async fn get_block<P: JsonRpcClient, B: Into<BlockId>>(provider: Provider<P>, block: B) -> anyhow::Result<()> {
  ...
  let filter = match block_number {
    BlockId::Hash(hash) => Filter::default().at_block_hash(hash),
    BlockId::Number(n) => Filter::default().from_block(n).to_block(n),
  };
